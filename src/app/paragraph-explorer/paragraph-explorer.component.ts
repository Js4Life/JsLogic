...
//>>>>>>> c8a8296b916971c558eeec0ed81b2a3d14ccf54c
  ...
  clickOnPara(para,e){ 
      ...
      if(this.clientState.selectedParaIndex){
        this.paragraphTempArray[this.clientState.selectedParaIndex].relatedParaOpen=false;
        this.paragraphTempArray[this.clientState.selectedParaIndex].showHide='SHOW';
        this.paragraphTempArray[this.clientState.selectedParaIndex].relatedParagraphs=[];
      }
    ...
    // if(!para.relatedParagraphs){
      ...
      this.analysisSvc.getRelatedParasInAnAnalysis(this.clientState.lastAnalysisId,para.id).then(data=>{
        ...
        this.relatedParaWithoutPrimaryFile=[];
        this.relatedParaWithPrimaryFile.forEach(obj=>{
          obj.contextDistribution = this.utilSvc.updateWeightAndColor(obj.contextDistribution);
            if(obj.documentName!=this.filesForAnalysis[0].name){
  ...
  showRelatedParas(para,e,idx){
    // this.clientState.selectedPara=para;
    // this.clientState.selectedParaIndex=idx;
    console.log('last para');
    console.log(this.clientState.selectedPara);
  ...
  changeFileSelection(idx, file){
    ...
    // console.log(this.paragraphTempArray[0]);
  ...
  extractParagraphs(){
    ...
    if(this.clientState.selectedParaIndex){
      this.clientState.selectedPara={};
      this.paragraphTempArray[this.clientState.selectedParaIndex].relatedParaOpen=false;
      this.paragraphTempArray[this.clientState.selectedParaIndex].showHide='SHOW';
      this.paragraphTempArray[this.clientState.selectedParaIndex].relatedParagraphs=[];
      this.clientState.selectedParaIndex='';
    }
    // this.clickOnPara(this.paragraphTempArray[0], event);
    if(this.clientState.CURRENT_STATE.includes('checklist-details')){
