...
export class FileExplorerComponent {
  ...
  updateUploadUrl(event: any, opt) {
    ...
    this.fileList = [];
     this.fileArray = [];
      this.popUp = true;
      this.fileArray[i] = event.target.files;
      this.fileArr = this.fileArray[i][i].name;
      this.fileList.push(this.fileArr); // get array of file names
      console.log('list',this.fileList);
      this.fileList.forEach(obj => {
        this.temp1 = {filename: obj};
        this.temp1.status = 'Uploading Report'; // testing
        this.temp1.percentage = this.randomGenerator(10, 50); // generate random vals
        this.fileDataArr.push(this.temp1);
    })
     // this.loading = true;
      ...
      //  this.loading = false;
        ...
        this.popUp = false; // disable popup
        this.timerfunc();
      //  this.showFinalPopup();
       // this.toastr.success('All Docs were updated successfully!');
  ...
  showFinalPopup() {
    if (this.uploadRes == 'File Uploaded') {
      console.log('res', this.uploadRes);
